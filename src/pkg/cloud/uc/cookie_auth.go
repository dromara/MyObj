...
func (m *CookieAuthManager) GetCookie(userID string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("未配置UC网盘Cookie")
	}
...
func (m *CookieAuthManager) GetConfig(userID string) (*CookieConfig, error) {
	...
	config, ok := m.configs[userID]
	if !ok {
		return nil, fmt.Errorf("未配置UC网盘Cookie")
	}
...
func (m *CookieAuthManager) ValidateCookie(cookie string) (bool, string, error) {
	// 调用UC API验证Cookie
	req, _ := http.NewRequest("GET", ucAPIBase+"/1/clouddrive/file/sort?pdir_fid=0&_page=1&_size=10&_sort=file_type:asc,updated_at:desc&pr=UCBrowser&fr=pc", nil)
	req.Header.Set("Cookie", cookie)
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) uc-cloud-drive/2.5.20 Chrome/100.0.4896.160 Electron/18.3.5.4-b478491100 Safari/537.36 Channel/pckk_other_ch")
...
func (m *CookieAuthManager) ListFiles(userID, dir string, page, size int) (map[string]interface{}, error) {
	...
	reqURL := fmt.Sprintf("%s/1/clouddrive/file/sort?pdir_fid=%s&_page=%d&_size=%d&_sort=file_type:asc,updated_at:desc&pr=UCBrowser&fr=pc",
		ucAPIBase, dir, page, size)
...
func (m *CookieAuthManager) GetShareDetail(userID, shareID, pwd string) (map[string]interface{}, error) {
	...
	reqURL := fmt.Sprintf("%s/1/clouddrive/share/sharepage/detail?pr=UCBrowser&fr=pc", ucAPIBase)
...
func (m *CookieAuthManager) GetDownloadURL(userID, fileID string) (string, error) {
	...
	reqURL := fmt.Sprintf("%s/1/clouddrive/file/download?pr=UCBrowser&fr=pc", ucAPIBase)
