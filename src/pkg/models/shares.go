...
type Share struct {
	...
	FileID        string               `gorm:"type:VARCHAR(255);not null" json:"file_id"`         // 文件ID
	Token         string               `gorm:"type:TEXT;not null" json:"-"`                       // 分享令牌
	ExpiresAt     custom_type.JsonTime `gorm:"type:DATETIME;not null" json:"expires_at"`          // 分享过期时间
	PasswordHash  string               `gorm:"type:TEXT;not null" json:"-"`                       // 访问密码哈希
