...
type FileInfo struct {
	ID              string               `gorm:"type:VARCHAR(255);not null;primaryKey;unique" json:"id"`                           // 文件ID，主键且唯一
	...
	Category        string               `gorm:"type:VARCHAR(255);default:'other';index:file_info_category_index" json:"category"` // 文件分类
	ThumbnailImg    string               `gorm:"type:TEXT" json:"thumbnail_img"`                                                   // 缩略图路径
	Path            string               `gorm:"type:TEXT" json:"path"`                                                            // 文件实际存储路径
	FileHash        string               `gorm:"type:VARCHAR(255);not null;index:file_info_file_hash_index" json:"file_hash"`      // 文件哈希值（全量hash）
	FileEncHash     string               `gorm:"type:TEXT" json:"file_enc_hash"`                                                   // 加密文件哈希值
	ChunkSignature  string               `gorm:"type:VARCHAR(255);index:file_info_chunk_signature_index" json:"chunk_signature"`   // 分片签名（快速预检）
	...
	EncPath         string               `gorm:"type:TEXT" json:"enc_path"`                                                        // 加密文件路径
